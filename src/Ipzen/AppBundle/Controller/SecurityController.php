<?php

namespace Ipzen\AppBundle\Controller;

use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use Symfony\Component\Security\Core\SecurityContext;
use Symfony\Component\HttpFoundation\Response;

class SecurityController extends Controller {
	
	/**
	 * Classic login action controller
	 *
	 */
	public function loginAction() {
		$request = $this->getRequest();
		$session = $request->getSession();

		// catch any login errors

		if ($request->attributes->has(SecurityContext::AUTHENTICATION_ERROR)) {

			$error = $request->attributes->get(SecurityContext::AUTHENTICATION_ERROR);

		} else {

			$error = $session->get(SecurityContext::AUTHENTICATION_ERROR); // should have nothing but we need the variable anyway
			$session->remove(SecurityContext::AUTHENTICATION_ERROR);
		}

		return $this->render('IpzenAppBundle:Security:login.html.twig', 
			array('last_username' => $session->get(SecurityContext::LAST_USERNAME),
				'error' => $error));
	}

	/**
	 * The logout action
	 * Symfony uses built-in logout mechanics, but doesn't seem to work for the time beeing
	 */
	public function logoutAction() {
		$this->get('security.context')->setToken(null);
		$this->get('request')->getSession()->invalidate();

		// ####################################
		// catch any login errors (will be made in a function)
		$request = $this->getRequest();
		$session = $request->getSession();
		
		if ($request->attributes->has(SecurityContext::AUTHENTICATION_ERROR)) {

			$error = $request->attributes->get(SecurityContext::AUTHENTICATION_ERROR);

		} else {

			$error = $session->get(SecurityContext::AUTHENTICATION_ERROR); // should have nothing but we need the variable anyway
			$session->remove(SecurityContext::AUTHENTICATION_ERROR);
		}

		return $this->render('IpzenAppBundle:Security:login.html.twig', array('last_username' => $session->get(SecurityContext::LAST_USERNAME),'error' => $error));

	}

	public function testAuthAction() {
		return $this->render('IpzenAppBundle:Security:testAuth.html.twig', array());
	}
}